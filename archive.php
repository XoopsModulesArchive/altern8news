...
// $Id: archive.php,v 1.1 2006/03/16 03:15:50 mikhail Exp $
//  ------------------------------------------------------------------------ //
...
$GLOBALS['xoopsOption']['template_main'] = 'news_archive.html';
...
$result = $xoopsDB->query('SELECT published FROM ' . $xoopsDB->prefix('atikel') . ' WHERE published>0 AND published<=' . time() . ' AND expired <= ' . time() . ' ORDER BY published DESC');
if (!$result) {
    exit();
}
    $years = [];
...
if (0 != $fromyear && 0 != $frommonth) {
    ...
    $sql = 'SELECT * FROM ' . $xoopsDB->prefix('atikel') . " WHERE published > $monthstart and published < $monthend ORDER by published DESC";
